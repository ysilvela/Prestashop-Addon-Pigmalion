...
class HttpRequestTest extends \Codeception\TestCase\Test
{
	...
	{
		$uri = TiresiasHttpRequest::buildUri(
	...
	{
		$url_parts = TiresiasHttpRequest::parseUrl('http://localhost:9000/tmp/?param1=first&param2=second#fragment1=test');
		$url = TiresiasHttpRequest::buildUrl($url_parts);
	...
	{
		$query_string_parts = TiresiasHttpRequest::parseQueryString('param1=first&param2=second');
	...
	{
		$url = TiresiasHttpRequest::replaceQueryParamInUrl('param1', 'replaced_first', 'http://localhost:9000/tmp/?param1=first&param2=second');
	...
	{
		$query_string = TiresiasHttpRequest::replaceQueryParam('param2', 'replaced_second', 'param1=first&param2=second');
	...
	{
		$response = new TiresiasHttpResponse(array(), json_encode(array('test' => true)));
	...
	{
		$response = new TiresiasHttpResponse(array(), '', 'error');
	...
	{
		$request = new TiresiasHttpRequest(new TiresiasHttpRequestAdapterCurl());
	...
	{
		$request = new TiresiasHttpRequest(new TiresiasHttpRequestAdapterSocket());
		$request->setUrl('http://localhost:3000');
		$response = $request->get();
		$this->assertEquals(404, $response->getCode());
		$response = $request->post('test');
